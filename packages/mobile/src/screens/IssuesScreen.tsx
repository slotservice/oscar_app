...
export function IssuesScreen({ route }: Props) {
  ...
  const loadIssues = async () => {
    try {
      const result = await api.issues.get(roundId);
  ...
  const addIssue = async () => {
    ...
    try {
      const result = await api.issues.create(roundId, {
      ...
      showAlert('Error', 'Failed to save issue.');
    }
  };
    ...
    <View style={commonStyles.screen}>
      ...
      <View style={styles.form}>
        ...
        <View style={commonStyles.rowBetween}>
          ...
          <Switch
            value={supervisorFlag}
            onValueChange={setSupervisorFlag}
            trackColor={{ false: colors.border, true: colors.primaryLight }}
            thumbColor={supervisorFlag ? colors.primary : colors.disabled}
          />
        </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.issueCard}>
            ...
            {item.actionTaken && (
              <Text style={styles.actionText}>Action: {item.actionTaken}</Text>
            )}
